...
class User extends Model
{
    ...
    public function update_age()
    {
        if (!empty($this->date_of_birth)) {
            ...
            $this->age = (date("md", date("U", mktime(0, 0, 0, $birthDate[0], $birthDate[1], $birthDate[2]))) > date("md") ? ((date("Y") - $birthDate[2]) - 1) : (date("Y") - $birthDate[2]));
        }
    }
    ...
    public function calcNorms()
    {
        $kcal = 0;
        if ($this->sex == 'woman') {
            $kcal = $this->weight * 10 + $this->height * 6.25 - $this->age * 5 - 161;
        } else {
            $kcal = $this->weight * 10 + $this->height * 6.25 - $this->age * 5 + 5;
        }
        $kcal=round($kcal * (float)$this->activity_level);
        //kcal other
        $this->norms['kcal']=$kcal;
        $this->norms['fat']=((3*$kcal)/10)/4;
        $this->norms['protein']=((2*$kcal)/10)/9;
        $this->norms['carbonation']=((5*$kcal)/10)/4;
        $this->norms['na']=2000;
